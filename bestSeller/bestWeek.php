...
<br>
    <?php		
		...
		$sl = $mysql->query("SELECT *, SUM(od.`item_quantity`) as sold_quantity FROM `book` b JOIN `order_detail` od ON b.book_id=od.book_id WHERE `order_id` IN (SELECT `order_id` FROM `order` WHERE YEARWEEK(order_time) = YEARWEEK(now())) GROUP BY od.`book_id` ORDER BY SUM(od.`item_quantity`) DESC");
		echo "Best seller of this week". "<br>". "<br>";
