...
<br>
    <?php		
		...
		$sl = $mysql->query("SELECT *, SUM(od.`item_quantity`) as sold_quantity FROM `book` b JOIN `order_detail` od ON b.book_id=od.book_id WHERE `order_id` IN (SELECT `order_id` FROM `order` WHERE YEAR(`order_time`) =YEAR(now()) AND MONTH(`order_time`) =MONTH(now())) GROUP BY od.`book_id` ORDER BY SUM(od.`item_quantity`) DESC");
		echo "Best seller of this month". "<br>". "<br>";
		echo "<table border=1 width='950'>";
		echo "<tr><td>Cover Page</td><td>Book Name</td><td>ISBN</td><td>Edition</td><td>Price</td><td>Quantity</td><td>Book Desc</td><td>Sold Quantity</td><td>Book Detail</td>";
